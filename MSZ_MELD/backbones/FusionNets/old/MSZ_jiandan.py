...
class MSZ(nn.Module):
    ...
    def forward(self, text_feats, video_feats, audio_feats, label_ids):
        ...
        t_a_att_con = self.get_con(self.att_ta(feats_output['text'], feats_output['audio'])['embedding'])
        t_v_att_con = self.get_con(self.att_tv(feats_output['text'], feats_output['video'])['embedding'])

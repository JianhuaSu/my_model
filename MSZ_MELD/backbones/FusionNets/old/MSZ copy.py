...
class MSZ(nn.Module):
    def __init__(self, args):
        ...
        self.text_feat_dim, self.video_feat_dim, self.audio_feat_dim = self.benchmarks['feat_dims']['text'], self.benchmarks['feat_dims']['audio'], self.benchmarks['feat_dims']['video']
        ...
        self.text_model = self.get_transformer_encoder(self.text_feat_dim, args.encoder_layers)
        self.text_liner = nn.Linear(self.text_feat_dim, self.hidden_dims)
        self.text_m_liner = nn.Linear(self.hidden_dims, self.hidden_dims)
        self.audio_model = self.get_transformer_encoder(self.audio_feat_dim, args.encoder_layers)
        self.audio_liner = nn.Linear(self.audio_feat_dim, self.hidden_dims)
        self.audio_m_liner = nn.Linear(self.hidden_dims, self.hidden_dims)
        self.video_model = self.get_transformer_encoder(self.video_feat_dim, args.encoder_layers)
        self.video_liner = nn.Linear(self.video_feat_dim, self.hidden_dims)
        self.video_m_liner = nn.Linear(self.hidden_dims, self.hidden_dims)
        ...
        self.f1_liner = nn.Linear(8 * self.hidden_dims, self.hidden_dims)
        self.f2_dropout = nn.Dropout(p=self.spe_text_dro)
        self.f2_liner = nn.Linear(self.hidden_dims, self.hidden_dims)
        
        # common liner
        self.common_liner = nn.Linear(self.hidden_dims, self.hidden_dims)
    ...
    def forward(self, text_feats, audio_feats, video_feats, label_ids):

        # text [16, 30, 768]
        text = self.text_bert(text_feats)
        text_per = text.permute(1, 0, 2)
        text = self.text_model(text_per)
        text = text.permute(1, 0, 2)        
        text = self.text_liner(text)
        text_mean = self.text_m_liner(text.mean(dim = 1))

        # video [16, 480, 768]
        video = video_feats.float()
        video_per = video.permute(1, 0, 2)
        video = self.video_model(video_per)
        video = video.permute(1, 0, 2)
        video = self.video_liner(video)
        video_mean = self.video_m_liner(video.mean(dim = 1))

        # audio [16, 230, 768]
        audio = audio_feats.float()
        audio_per = audio.permute(1, 0, 2)
        audio = self.audio_model(audio_per)
        audio = audio.permute(1, 0, 2)
        audio = self.audio_liner(audio)
        audio_mean = self.audio_m_liner(audio.mean(dim = 1))

        # attention [16,768]
        t_a_att = self.t_a_att_liner(self.t_a_att_dropout(self.att(text, audio)))
        a_t_att = self.a_t_att_liner(self.a_t_att_dropout(self.att(audio, text)))
        t_v_att = self.t_v_att_liner(self.t_v_att_dropout(self.att(text, audio)))
        v_t_att = self.v_t_att_liner(self.v_t_att_dropout(self.att(video, text)))
        ...
        # fusion
        fusion_feats = torch.cat((self.common_liner(t_a_att), self.common_liner(a_t_att), self.common_liner(t_v_att), self.common_liner(v_t_att), self.common_liner(t_v_spe_feat), self.common_liner(t_a_spe_feat), self.common_liner(a_spe_feat), self.common_liner(v_spe_feat)), dim = 1)
        ...
        # loss = self.args.sl * specific_loss + self.args.cl * (ta_c_loss + tv_c_loss) + fusion_loss

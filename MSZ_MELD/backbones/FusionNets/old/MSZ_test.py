...
class MSZ(nn.Module):
    def __init__(self, args):
        ...
        self.cos_class = CosNorm_Classifier(3*768, 3*768, args.scale, args.device)

        # transformer encoder
        self.text_bert = BERTEncoder.from_pretrained(args.text_backbone_path)
        self.text_liner = nn.Linear(2*self.text_feat_dim, self.hidden_dims)
        self.audio_liner = nn.Linear(2*self.audio_feat_dim, self.hidden_dims)
        self.video_liner = nn.Linear(2*self.video_feat_dim, self.hidden_dims)
    ...
    def get_feats(self, text, audio, video):
        ...
        seq_size = text.size(1)
        hidden_size = text.size(2)

        text_tag = torch.ones(batch_size, seq_size, hidden_size, device=self.args.device)
        video_tag = torch.ones(batch_size, seq_size, 256, device=self.args.device) * 2
        audio_tag = torch.ones(batch_size, seq_size, hidden_size, device=self.args.device) * 3

        text_concat = torch.cat((text, text_tag), dim=2)
        video_concat = torch.cat((video, video_tag), dim=2)
        audio_concat = torch.cat((audio, audio_tag), dim=2)

        text = self.att_t(self.text_liner(text_concat))
        audio = self.att_a(self.audio_liner(audio_concat.float()))
        video = self.att_v(self.video_liner(video_concat.float()))
    ...
    def forward(self, text_feats, video_feats, audio_feats, label_ids):
        ...
        fusion_feats = self.cos_class(fusion_feats)

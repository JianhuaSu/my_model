...
class MSZ(nn.Module):
    ...
    def get_feats(self, text, audio, video):
        ...
        if self.args.ablation == 'no_text':
            text = torch.zeros_like(text)
        if self.args.ablation == 'no_video':
            audio = torch.zeros_like(audio)
        if self.args.ablation == 'no_audio':
    ...
    def get_ablation_feats(self, text, audio, video):
        ...
        audio = self.audio_liner(audio.float())
        video = self.video_liner(video.float())

        output = {
            'text': self.layernorm(self.activation(text)),
            'audio': self.layernorm(self.activation(audio)),
            'video': self.layernorm(self.activation(video))
        }
    ...
    def forward(self, text_feats, video_feats, audio_feats, label_ids):
        ...
        if self.args.ablation == 'no_selfatt':
            feats_output = self.get_ablation_feats(text_feats, audio_feats, video_feats)
        else:    
            feats_output = self.get_feats(text_feats, audio_feats, video_feats)
        
        if self.args.ablation == 'no_modality':
            text, video, audio = feats_output['text'],feats_output['video'], feats_output['audio']
        else:
        ...
        if self.args.ablation == 'no_modality':
            fusion_ta = self.layernorm(self.activation(fusion_ta))
            fusion_tv = self.layernorm(self.activation(fusion_tv))
            t_a_att_con = self.att_ta(fusion_tv, text)['embedding']
            t_v_att_con = self.att_tv(fusion_ta, text)['embedding']
            t_con = self.att_t(text)['embedding']
        else:
        ...
        if self.args.ablation == 'no_cross':
            fusion_feats = torch.cat((torch.tensor(fusion_ta[:, -1, :]), torch.tensor(fusion_tv[:, -1, :]), torch.tensor(self.t_liner(text))[:, -1, :]), dim=1)
        else:

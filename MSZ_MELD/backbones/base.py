...
class MIA(nn.Module):
    ...
    def forward(self, text_feats, video_feats, audio_feats, label_ids=None, mode=None):
        
        # video_feats = self.video_mae(video_feats)
        if self.args.method != 'mmim':
        ...
        elif self.args.method == 'mmim':
            mm_model = self.model(text_feats, video_feats, audio_feats, mode)
        else:

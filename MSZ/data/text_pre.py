...
class TextDataset:
    ...
    def _get_feats(self, args, base_attrs):
        ...
        train_examples = processor.get_examples(base_attrs['data_path'], 'train', args.dataset)
        train_feats = self._get_bert_feats(args, train_examples, base_attrs)

        dev_examples = processor.get_examples(base_attrs['data_path'], 'dev', args.dataset)
        dev_feats = self._get_bert_feats(args, dev_examples, base_attrs)

        test_examples = processor.get_examples(base_attrs['data_path'], 'test', args.dataset)
    ...
    def _get_bert_feats(self, args, examples, base_attrs):
        ...
        if args.text_backbone.startswith('bert'):
            tokenizer = BertTokenizer.from_pretrained(args.text_backbone_path, do_lower_case=True)   
...
class DatasetProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, mode, dataset):
        if mode == 'train':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "train.tsv")), "train", dataset)
        elif mode == 'dev':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "dev.tsv")), "train", dataset)
        elif mode == 'test':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "test.tsv")), "test", dataset)

    def _create_examples(self, lines, set_type, dataset):

...
class MSZ(nn.Module):
    def __init__(self, args):
        ...
        self.fusion_layer_b = nn.Linear(768, self.hidden_dims)
        self.fusion_layer_b_dropout =nn.Dropout(p=self.spe_text_dro)
        self.classifier_b = nn.Linear(self.hidden_dims, self.output_dim)
    ...
    def get_logits_b(self, h):
        
        h = self.fusion_layer_b(h)
        h = self.sigmoid(h)
        h_last = self.fusion_layer_b_dropout(h)
  
        logits = self.classifier_b(h_last)
    ...
    def forward(self, text_feats, video_feats, audio_feats, label_ids):
        ...
        t_a_att_con = self.get_t(self.att_ta(audio, text))
        # a_t_att_con = self.get_a(a_t_att)

        t_v_att_con = self.get_t(self.att_tv(video, text))
        ...
        fusion_feats = torch.cat((t_v_att_con ,t_a_att_con, text_mean), dim=1)
        ...
        # loss = ta_c_loss*0.05 + fusion_loss

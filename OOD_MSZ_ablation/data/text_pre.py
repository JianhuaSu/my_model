...
def get_t_data(args, data_args, ood):
    ...
    if args.text_backbone.startswith('bert') or args.text_backbone.startswith('roberta'):
    ...
    def __init__(self, args, ood = False):
        ...
        if not ood:
            if args.dataset in ['MIntRec']:
                self.select_id = 3
            elif args.dataset in ['MIntRec2','MELD-DA']:
                self.select_id = 2

        else:
            if args.ood_dataset in ['MIntRec', 'MIntRec-OOD']:
                self.select_id = 3
            elif args.ood_dataset in ['MIntRec2', 'MIntRec2-OOD','MELD-DA', 'MELD-DA-OOD']:
                self.select_id = 2

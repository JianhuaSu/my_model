...
class BERT(BertPreTrainedModel):
    def __init__(self, config, args):
        ...
        self.n_classifier = nn.Linear(config.hidden_size, args.num_labels)
        self.c_classifier = nn.Linear(config.hidden_size, args.num_labels)
    ...
    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, labels=None,
        ...
        n_logits = self.n_classifier(pooled_output)
        c_logits = self.c_classifier(pooled_output)

        if feature_ext:
            return pooled_output
        else:
            if mode == 'train':
                loss_ce = loss_fct(n_logits, labels)
                return loss_ce,pooled_output,n_logits,c_logits
            else:
                return pooled_output, n_logits

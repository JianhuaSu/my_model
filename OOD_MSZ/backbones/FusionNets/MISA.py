...
class MISA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.project_v.add_module('project_v', nn.Linear(in_features=self.visual_size * 4, out_features=args.hidden_size))
        ...
        self.fusion_layer_1 = nn.Linear(in_features=args.hidden_size * 6, out_features=args.hidden_size * 3)
        self.fusion_layer_1_dropout = nn.Dropout(self.dropout_rate)
        self.fusion_layer_1_activation = self.activation
        self.fusion_layer_3 = nn.Linear(in_features=args.hidden_size * 3, out_features= self.output_dim)
        ...
        args.feat_size = args.hidden_size * 3
        args.text_feat_size = self.text_size
        args.video_feat_size = self.visual_size * 4
        args.audio_feat_size = hidden_sizes[2] * 4
    ...
    def forward(self, text_feats, video_feats, audio_feats, labels = None, ood_sampler = None, device = None):
        
        video_f, audio_f = video_feats.float(), audio_feats.float()
        sent, sent_mask, sent_type = text_feats[:,0], text_feats[:,1], text_feats[:,2]
        text_f = self.text_subnet(text_feats) # [batch_size, seq_len, 768]

        # Use the mean value of bert/roberta of the front real sentence length as the final representation of text.
        masked_output = torch.mul(sent_mask.unsqueeze(2), text_f)
        mask_len = torch.sum(sent_mask, dim=1, keepdim=True)  
        text = torch.sum(masked_output, dim=1, keepdim=False) / mask_len

        batch_size = text.size(0)
        lengths = mask_len.squeeze().int().detach().cpu().view(-1)

        # extract features from visual modality
        final_h1v, final_h2v = self._extract_features(video_f, lengths, self.vrnn1, self.vrnn2, self.vlayer_norm)
        video = torch.cat((final_h1v, final_h2v), dim=2).permute(1, 0, 2).contiguous().view(batch_size, -1)

        # extract features from acoustic modality
        final_h1a, final_h2a = self._extract_features(audio_f, lengths, self.arnn1, self.arnn2, self.alayer_norm)
        audio = torch.cat((final_h1a, final_h2a), dim=2).permute(1, 0, 2).contiguous().view(batch_size, -1)

        self._shared_private(text, video, audio)
        ...
        h = self.fusion_layer_1_dropout(h)
        h_last = self.fusion_layer_1_activation(h)
        logits = self.fusion_layer_3(h_last)
    ...
    def vim(self):
        ...
        return self.fusion_layer_3.weight, self.fusion_layer_3.bias
